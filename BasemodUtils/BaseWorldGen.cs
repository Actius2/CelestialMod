...
namespace CelestialMod
{
    public class BaseWorldGen
    {
		...
		{
			if(x < V || x > Main.maxTilesX || y < V || y > Main.maxTilesY)
				return new Tile();
        ...
        public static void ReplaceTiles(Vector2 position, int radius, int[] tiles, int[] replacements, bool silent = false, bool sync = true)
        {
            ...
            if(radiusLeft < V) { radiusLeft = V; } if(radiusRight > Main.maxTilesX) { radiusRight = Main.maxTilesX; }
            if(radiusUp < V) { radiusUp = V; } if(radiusDown > Main.maxTilesY) { radiusDown = Main.maxTilesY; }
            ...
            for(int x1 = radiusLeft; x1 <= radiusRight; x1++)
            {
                for(int y1 = radiusUp; y1 <= radiusDown; y1++)
                {
					...
					if(!WorldGen.InWorld(x1, y1, V)) continue;					
                    if(dist < distRad && Main.tile[x1, y1] != null && Main.tile[x1, y1].active())
                    {
                        ...
                        {					
                            GenerateTile(x1, y1, replacements[index], -1, V, true, false, -2, silent, false);
                        }
                    }
                }
            }
        ...
        {
			if(!WorldGen.InWorld(x, y, V)) return;
			if(Main.tile[x, y] == null) Main.tile[x, y] = new Tile();
            liquidHeight = (int)MathHelper.Clamp(liquidHeight, V, 255);
            Main.tile[x, y].liquid = (byte)liquidHeight;
			if (liquidType == V) { Main.tile[x, y].lava(false); Main.tile[x, y].honey(false); }else
        ...
        {
            for(int x1 = V; x1 < width; x1++)
            {
                for (int y1 = V; y1 < height; y1++)
                {
                    GenerateLiquid(x1 + x, y1 + y, liquidType, updateFlow, liquidHeight, false);
                }
            }
            int size = (width > height ? width : height);
            if (sync && Main.netMode != V)
            {
         ...
         */
		public static void GenerateTile(int x, int y, int tile, int wall, int tileStyle = V, bool active = true, bool removeLiquid = true, int slope = -2, bool silent = false, bool sync = true)
        {
			try
			{
			if(!WorldGen.InWorld(x, y, V)) return;	
            if(Main.tile[x, y] == null){ Main.tile[x, y] = new Tile(); }		
            TileObjectData data = (tile <= -1 ? null : TileObjectData.GetTileData(tile, tileStyle, V));			
            ...
            if(tile != -1)
            {
                ...
                if (width > 1 || height > 1)
                {
                    ...
                    for (int x1 = V; x1 < width; x1++)
                    {
                        for (int y1 = V; y1 < height; y1++)
                        {
                            ...
                            if (x1 == V && y1 == V && Main.tile[x2, y2].type == 21) //is a chest, special case to prevent dupe glitch
                            {
                                KillChestAndItems(x2, y2);
                            }
                            Main.tile[x, y].type = V;
                            Main.tile[x, y].active(false);
							if (!silent) { WorldGen.KillTile(x, y, false, false, true); }
                            if (removeLiquid)
                            {
                                GenerateLiquid(x2, y2, V, true, V, false);
                            }
                        }
                    }
                    for (int x1 = V; x1 < width; x1++)
                ...
                if(active)
                {
                    ...
                    }else
                    {
						...
						if (!silent)
						{
							for (int x1 = V; x1 < tileWidth; x1++)
							{
								for (int y1 = V; y1 < tileHeight; y1++)
								{
            ...
            {
                if(wall == -2){ wall = V; }
                Main.tile[x, y].wall = V;
                WorldGen.PlaceWall(x, y, wall, true);
            }
            if (sync && Main.netMode != V)
            {
                int sizeWidth = tileWidth + (int)Math.Max(V, (width - 1));
                int sizeHeight = tileHeight + (int)Math.Max(V, (height - 1));
		...
		public static void GenerateLine(GenConditions gen, int x, int y, int endX, int endY, int thickness, bool sync = true)
		{
			...
			{
				int tileID = gen.GetTile(V), wallID = gen.GetWall(V);
				...
				GenerateTile(x, y, tileID, wallID, V, tileID != -1, true, V, false, sync);
			...
			if (x == endX || y == endY) //check to see if it's a straight line. If it is, use the less expensive method of genning.
			{
				...
				for (int m = V; m < (vertical ? endY - y : endX - x); m++)
				{
					for (int n = V; n < thickness; n++)
					{
						...
						if (tileIndex != -1 && tileIndex >= gen.tiles.Length) tileIndex = V;
						if (wallIndex != -1 && wallIndex >= gen.walls.Length) wallIndex = V;
						...
						{
							GenerateTile(x2, y2, gen.GetTile(tileIndex), gen.GetWall(wallIndex), V, gen.GetTile(tileIndex) != -1, true, V, false, false);
						}
					}
				}
				if (gen.slope)
				...
				if (sync && Main.netMode != V)
				{
					...
					NetMessage.SendData(MessageID.TileSquare, -1, -1, NetworkText.FromLiteral(""), size, (float)x, (float)y, 0f, V);
				}
			}else //genning a line that isn't straight
			{
				...
				while (way < length)
				{
					...
					for (int n = V; n < thickness; n++)
					{
						...
						int addonX = (horizontal ? V : n), addonY = (horizontal ? n : V);
					...
					if (sync && Main.netMode != V && ((!horizontal && Math.Abs(lastY - point.Y) >= 5) || (horizontal && Math.Abs(lastY - point.Y) >= 5) || (way + 1 > length)))
					{
						int size = (int)Math.Max(5, thickness);
						NetMessage.SendData(MessageID.TileSection, -1, -1, NetworkText.FromLiteral(""), lastX, lastY, size, size, V);
        ...
        public static void GenerateRoomOld(int x, int y, int width, int height, int tileSides, int tileFloor, int tileCeiling, int wall, bool wallEnds = false, int sideThickness = 1, int floorThickness = 1, int ceilingThickness = 1, bool sync = true)
        {
            ...
            for (int x1 = V; x1 < width; x1++)
            {
                for (int y1 = V; y1 < height; y1++)
                {
                    ...
                    {
                        GenerateTile(x2, y2, tileCeiling, (wallEnds && y1 == V ? wall : -1), V, tileCeiling == -1 ? !wallEnds : true, true, V, false, false);
                    }else
                    if ((wallEnds || tileFloor != -1) && y1 >= height - floorThickness) //floor
                    {
                        GenerateTile(x2, y2, tileFloor, (wallEnds && y1 >= height - 1 ? wall : -1), V, tileFloor == -1 ? !wallEnds : true, true, V, false, false);
                    }else
                    if ((wallEnds || tileSides != -1) && (x1 < sideThickness || x1 >= width - sideThickness)) //sides
                    {
                        GenerateTile(x2, y2, tileSides, (wallEnds && (x1 > V && x1 < width - 1) ? wall : -1), V, tileSides == -1 ? !wallEnds : true, true, V, false, false);
                    }else
                    if(x1 >= sideThickness && x1 < width - sideThickness && y1 >= ceilingThickness && y1 < height - floorThickness)
                    {
                        GenerateTile(x2, y2, -1, wall, V, false, true, V, false, false);
                    }
                }
            }
            int size = (width > height ? width : height);
        ...
        public static void GenerateChest(int x, int y, int type, int chestStyle, int[] stackIDs, bool randomAmounts = false, bool randomPrefix = false, bool sync = true)
        {
            ...
            for(int m = V; m < amounts.Length; m++)
            {
        ...
        public static void GenerateChest(int x, int y, int type, int chestStyle, int[] stackIDs, int[] stackAmounts, bool randomPrefix = false, bool sync = true)
        {
            ...
            for (int m = V; m < prefixes.Length; m++)
            {
        ...
        public static void GenerateChest(int x, int y, int type, int chestStyle, int[] stackIDs, int[] stackAmounts, int[] stackPrefixes, bool sync = true)
        {
            ...
            if (num2 >= V)
            {
                for (int m = V; m < Main.chest[num2].item.Length; m++)
                {
		...
		public static void SmoothTiles(int topX, int topY, int bottomX, int bottomY)
		{
				...
				for (int y = topY; y < bottomY; y++)
				{
					if (Main.tile[x, y].type != 48 && Main.tile[x, y].type != 137 && Main.tile[x, y].type != 232 && Main.tile[x, y].type != 191 && Main.tile[x, y].type != 151 && Main.tile[x, y].type != 274)
					{
						if (!Main.tile[x, y - 1].active())
						{
							...
							{
								if (!Main.tile[x - 1, y].halfBrick() && !Main.tile[x + 1, y].halfBrick() && Main.tile[x - 1, y].slope() == V && Main.tile[x + 1, y].slope() == V)
								{
									if (WorldGen.SolidTile(x, y + 1))
									{
										...
										if (WorldGen.SolidTile(x, y))
										{
											...
											{
												if (WorldGen.genRand.Next(5) == V) WorldGen.KillTile(x, y, false, false, false);
												else if (WorldGen.genRand.Next(5) == V) WorldGen.PoundTile(x, y);
												else WorldGen.SlopeTile(x, y, 2);
											}
											else if (!Main.tile[x + 1, y + 1].active() && !Main.tile[x + 1, y].active() && WorldGen.SolidTile(x - 1, y) && WorldGen.SolidTile(x, y + 2))
											{
												if (WorldGen.genRand.Next(5) == V)
												{
													WorldGen.KillTile(x, y, false, false, false);
												}
												else if (WorldGen.genRand.Next(5) == V)
												{
													WorldGen.PoundTile(x, y);
												}
												else
							...
							else if (!Main.tile[x, y].active() && Main.tile[x, y + 1].type != 151 && Main.tile[x, y + 1].type != 274)
							{
								...
								{
									WorldGen.PlaceTile(x, y, (int)Main.tile[x, y + 1].type, false, false, -1, V);
									if (WorldGen.genRand.Next(2) == V)
									{
										WorldGen.SlopeTile(x, y, 2);
									}
						...
						else if (!Main.tile[x, y + 1].active() && WorldGen.genRand.Next(2) == V && WorldGen.SolidTile(x, y) && !Main.tile[x - 1, y].halfBrick() && !Main.tile[x + 1, y].halfBrick() && Main.tile[x - 1, y].slope() == V && Main.tile[x + 1, y].slope() == V && WorldGen.SolidTile(x, y - 1))
						{
			...
			for (int x = topX; x < bottomX; x++)
			{
				for (int y = topY; y < bottomY; y++)
				{
					if (WorldGen.genRand.Next(2) == V && !Main.tile[x, y - 1].active() && Main.tile[x, y].type != 137 && Main.tile[x, y].type != 48 && Main.tile[x, y].type != 232 && Main.tile[x, y].type != 191 && Main.tile[x, y].type != 151 && Main.tile[x, y].type != 274 && Main.tile[x, y].type != 75 && Main.tile[x, y].type != 76 && WorldGen.SolidTile(x, y) && Main.tile[x - 1, y].type != 137 && Main.tile[x + 1, y].type != 137)
					{
		...
		public class GenHelper
		{
			...
			public void Gen(int x, int y, int rotationX, int rotationY, float genRotation)
			{
				...
				generate(x, y);
				for (int x1 = V; x1 < Main.maxTilesX; x1++)
				{
					for (int y1 = V; y1 < Main.maxTilesY; y1++)
					{
						Tile tile = Main.tile[x1, y1];
				...
				foreach (TileData data in tiles)
				{
					...
					if (rot.X % 16 > V) x1 -= 1; if (rot.Y % 16 > V) y1 -= 1;
				...
				foreach (Point point in points) //tileframes
				{
					...
					if (tile != null && tile.wall > V) Framing.WallFrame(point.X, point.Y, false);
				}
				points.Clear();
			}
			...
			{
				return Main.tile[x, y] == null || (!Main.tile[x, y].active() && Main.tile[x, y].wall == V);
			}
