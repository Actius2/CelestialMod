                        ...
                        if (dist < (double)explosionIntensity)
                        {
                            ...
                            if (Main.tile[x2, y2] != null && Main.tile[x2, y2].active())
                            {
                                ...
                                if (canExplode)
                                {
                                    ...
                                    if (sync && !Main.tile[x2, y2].active() && Main.netMode != NetmodeID.SinglePlayer) { NetMessage.SendData(BaseConstants.NET_TILE_UPDATE, -1, -1, NetworkText.FromLiteral(""), 0, (float)x2, (float)y2, 0f, 0); }
                                }
                            }
                            if (canExplode)
                            {
                                for (int x3 = x2 - 1; x3 <= x2 + 1; x3++)
                                {
                                    for (int y3 = y2 - 1; y3 <= y2 + 1; y3++)
                                    {
                                        if (Main.tile[x3, y3] != null && Main.tile[x3, y3].wall > 0 && updateWalls)
                                        {
                                            ...
                                            if(sync && Main.tile[x3, y3].wall == 0 && Main.netMode != NetmodeID.SinglePlayer) { NetMessage.SendData(BaseConstants.NET_TILE_UPDATE, -1, -1, NetworkText.FromLiteral(""), 2, (float)x3, (float)y3, 0f, 0); }
                                        }
                                    }
                                }
                            }
